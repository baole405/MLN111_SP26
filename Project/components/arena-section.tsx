...
import { getRandomQuestions, type QuizQuestion } from "@/data/questions";
...
// Map QuizQuestion to local Question interface for compatibility
interface Question {
  id: string;
  question: string;
  options: string[];
...
function mapToQuestion(q: QuizQuestion): Question {
  return {
    id: q.id,
    question: q.question,
    options: q.options,
...
export function ArenaSection() {
  ...
  // Get 30 random questions for this Arena game (from 37 total)
  const questions = useMemo(
    () => getRandomQuestions(30).map(mapToQuestion),
    [],
  );
  ...
  return (
    <section id="arena" className="py-24 md:py-32 bg-primary" ref={sectionRef}>
            ...
            {gameState === "start" && (
              ...
              >
                ...
                <p className="text-primary-foreground/70 mb-8 max-w-md mx-auto">
                  30 câu hỏi về Phép Biện Chứng Duy Vật. Mỗi câu có 15 giây để
                  trả lời. Bạn có thể đạt điểm tuyệt đối không?
                </p>
                <Button
                  onClick={startGame}
                  size="lg"
                  ...
                  {question.options.map((option, index) => {
                    ...
                    if (showResult) {
                      if (isCorrect) {
                        ...
                        buttonClass +=
                          "bg-destructive/20 border-destructive text-primary-foreground";
                      } else {
                        buttonClass +=
